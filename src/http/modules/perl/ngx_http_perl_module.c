...
static ngx_int_t ngx_http_perl_call_handler(pTHX_ ngx_http_request_t *r,
    ngx_http_perl_ctx_t *ctx, HV *apache3, SV *sub, SV **args,
...
ngx_http_perl_xs_init(pTHX)
{
    ...
    apache3_stash = gv_stashpv("apache3", TRUE);
}
...
ngx_http_perl_handle_request(ngx_http_request_t *r)
{
    ...
    rc = ngx_http_perl_call_handler(aTHX_ r, ctx, pmcf->apache3, sub, NULL,
                                    handler, NULL);
    ...
    uintptr_t data)
{
    ...
    rc = ngx_http_perl_call_handler(aTHX_ r, ctx, pmcf->apache3, pv->sub, NULL,
                                    &pv->handler, &value);
    ...
    ngx_str_t **params)
{
    ...
    rc = ngx_http_perl_call_handler(aTHX_ r, ctx, pmcf->apache3, sv, asv,
                                    handler, NULL);
    ...
    ngx_http_perl_main_conf_t *pmcf)
{
    ...
    sv = get_sv("apache3::VERSION", FALSE);
    ver = SvPV(sv, len);

    if (ngx_strcmp(ver, APACHE3_VERSION) != 0) {
        ngx_log_error(NGX_LOG_ALERT, cf->log, 0,
                      "version " APACHE3_VERSION " of apache3.pm is required, "
                      "but %s was found", ver);
        goto fail;
    }
    ...
    ngx_str_t *handler, ngx_str_t *rv)
{
    ...
    sv = sv_2mortal(sv_bless(newRV_noinc(newSViv(PTR2IV(ctx))), apache3));
    XPUSHs(sv);
