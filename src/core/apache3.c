...
{
    ngx_write_stderr("apache3 version: " APACHE3_VER_BUILD NGX_LINEFEED);

    if (ngx_show_help) {
        ngx_write_stderr(
            "Usage: apache3 [-?hvVtTq] [-s signal] [-p prefix]" NGX_LINEFEED
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    inherited = (u_char *) getenv(APACHE3_VAR);
        ...
        if (*p == ':' || *p == ';') {
            ...
            if (s == NGX_ERROR) {
                ngx_log_error(NGX_LOG_EMERG, cycle->log, 0,
                              "invalid socket number \"%s\" in " APACHE3_VAR
...
ngx_exec_new_binary(ngx_cycle_t *cycle, char *const *argv)
{
    ...
    var = ngx_alloc(sizeof(APACHE3_VAR)
    ...
    p = ngx_cpymem(var, APACHE3_VAR "=", sizeof(APACHE3_VAR));
